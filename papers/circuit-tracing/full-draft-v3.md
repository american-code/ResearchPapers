...
> **REVISION NOTES (v3)** — v3 integrates circuit-v2 results: Llama-3.1-8B-Instruct circuit
> (Section 4.6), circuit faithfulness and completeness metrics (Sections 3.6, 4.7).
> Pythia negative faithfulness (F = −0.056) is a verified empirical finding; addressed
> in the Discussion placeholder. All previously open items below remain unresolved.
>
...
5. **Circuit faithfulness evaluation reveals a necessity–sufficiency dissociation.** Applying circuit faithfulness metrics (Section 3.6) to Llama-3.1-8B-Instruct and Pythia-1.4b, we find that Llama-3.1-8B achieves partial positive faithfulness (F = 0.265) while Pythia's top-10 circuit has negative faithfulness (F = −0.056) — the isolated circuit actively predicts the wrong token. In both models the circuit heads are genuinely necessary (completeness F ≈ 0.24–0.32), but the activation-patching selection procedure does not guarantee a collectively sufficient circuit. The dominant head in each model (L19H23 in Llama-3.1-8B, L10H7 in Pythia) is correctly identified, but composing the full top-N set fails for Pythia due to context-dependent suppressive heads that require the broader model to function correctly.
...
### 3.6 Circuit Faithfulness and Completeness

Per-head patching scores measure individual sufficiency: how much a single head's clean activation, substituted into a corrupted run, recovers the target behavior. They do not directly address whether a *set* of heads constitutes a circuit in the stronger sense — a subset of components that, in isolation, can reproduce the model's clean behavior.

We evaluate this circuit-level question using two complementary metrics:

**Circuit faithfulness.** For a proposed circuit $C$ (a set of $(l, h)$ pairs), define:

$$F(C) = \frac{\overline{\text{LD}}_{C\text{-only}} - \overline{\text{LD}}_{\text{all-ablated}}}{\overline{\text{LD}}_{\text{clean}} - \overline{\text{LD}}_{\text{all-ablated}}}$$

where $\overline{\text{LD}}_{C\text{-only}}$ is the mean logit difference when all heads *not in $C$* are mean-ablated. A faithfulness score of 1.0 means the circuit alone fully recovers clean performance; 0.0 means no recovery beyond the all-ablated baseline; negative values mean the isolated circuit actively predicts the subject over the IO — the wrong answer.

**Completeness (joint ablation).** The complementary necessity measure:

$$F_{\text{complete}}(C) = \frac{\overline{\text{LD}}_{\text{model}\setminus C} - \overline{\text{LD}}_{\text{all-ablated}}}{\overline{\text{LD}}_{\text{clean}} - \overline{\text{LD}}_{\text{all-ablated}}}$$

where $\overline{\text{LD}}_{\text{model}\setminus C}$ is the mean LD when all heads in $C$ are ablated from the full model. $F_{\text{complete}}$ close to 1.0 indicates the rest of the model can largely do the task without the circuit. A pair $(F, F_{\text{complete}})$ where both are low indicates a tight, faithful circuit; where $F_{\text{complete}}$ is high and $F$ is low, the heads are neither sufficient alone nor critical to the full model.

**Leave-one-in analysis.** To assess standalone head contributions within the circuit, we compute $F_{\{h\}}$: mean-ablate all circuit heads except $h$ and report the resulting faithfulness. Heads with $F_{\{h\}} > F(C)$ are self-sufficient contributors; heads with $F_{\{h\}} < F_{\text{complete}}$ actively interfere when left alone.

These metrics were applied to Llama-3.1-8B-Instruct and Pythia-1.4b on a v2 dataset of $n = 200$ examples drawn from 15 IOI templates (one template was used in the v1 analysis; see Section 3.7), to test robustness of the selection to template variation. Data: `data/circuit-v2/faithfulness-llama8b-v2.json`, `data/circuit-v2/faithfulness-pythia-v2.json`.
...
### 4.6 Extension to Llama-3.1-8B-Instruct

We applied the same activation patching selection protocol (top-10 by mean normalized LD recovery, $n = 200$ examples, 15 templates; v2 dataset) to Llama-3.1-8B-Instruct (`mlx-community/Meta-Llama-3.1-8B-Instruct-4bit`; 32 layers, 32 heads, dense MHA). This is the largest model in this paper at 8 billion parameters, versus 3 billion for Llama-3.2-3B and 1.4 billion for Pythia.

Mean clean logit difference is 5.229 (comparable to Llama-3.2-3B's 5.649); the all-ablated baseline is −0.046. Table 6 reports the circuit heads ordered by leave-one-in faithfulness.

**Table 6.** Top-10 circuit-critical heads for Llama-3.1-8B-Instruct by activation patching score (n = 200 examples, 15 templates), with leave-one-in faithfulness. Rel. depth = layer / 32. ΔF = F_leave_in − F_complete; positive values indicate the head is a positive standalone contributor. Data: `data/circuit-v2/faithfulness-llama8b-v2.json`, `data/circuit-v2/joint-ablation-llama8b-v2.json`.

| Leave-in rank | Head    | Rel. depth | F_leave_in | ΔF (vs. F_complete = 0.236) |
|---------------|---------|------------|------------|----------------------------|
| 1             | L19H23  | 0.594      | 0.4585     | +0.222                     |
| 2             | L17H27  | 0.531      | 0.3746     | +0.139                     |
| 3             | L16H1   | 0.500      | 0.3066     | +0.071                     |
| 4             | L30H13  | 0.938      | 0.3031     | +0.067                     |
| 5             | L17H24  | 0.531      | 0.2842     | +0.048                     |
| 6             | L24H27  | 0.750      | 0.2722     | +0.036                     |
| 7             | L22H19  | 0.688      | 0.2665     | +0.031                     |
| 8             | L20H3   | 0.625      | 0.2550     | +0.019                     |
| 9             | L16H22  | 0.500      | 0.2346     | −0.001                     |
| 10            | L27H20  | 0.844      | 0.2150     | −0.021                     |

**Circuit faithfulness: F = 0.265.** The 10-head circuit recovers the LD from −0.046 (all-ablated) to 1.354 against a clean target of 5.229 — 26.5% of the clean signal. Completeness: $F_	ext{complete} = 0.236$; removing these 10 heads from the full model drops LD to 1.198. The two metrics are close to each other (F ≈ F_complete), which means this circuit is approximately as important to the full model as it is faithful in isolation — a tighter characterization than Pythia (Section 4.7).

**L19H23 as dominant head.** At $F_{\{h\}} = 0.459$, L19H23 (relative depth 0.594) alone recovers nearly as much as the full 10-head circuit. It is also the most necessary head: removing it from the circuit reduces faithfulness from 0.265 to 0.133 (ΔF minimality = +0.132). This double dominance — highest leave-in score, largest minimality drop — identifies L19H23 as the bottleneck head for the 8B model, analogous to L15H20 in Llama-3.2-3B and L10H7 in Pythia (Section 4.2–4.3).

**Suppressive or context-dependent heads.** L16H22 and L27H20 have negative ΔF: leaving either as the sole active head yields lower performance than the F_complete baseline. These heads are individually detrimental when isolated, suggesting their role is facilitative or suppressive rather than directly IO-promoting, and that it operates only in context of the other circuit heads. L17H27 also deserves note: removing it from the circuit *increases* faithfulness (F_without = 0.357 vs. full-circuit F = 0.265; minimality ΔF = −0.092), making L17H27 a net-negative contributor to the isolated circuit despite its high leave-one-in score. This dissociation — strong standalone contribution but detrimental to the group — suggests L17H27 performs a computation that is redundant with or interferes with other circuit heads when combined.

**Depth distribution.** All 10 circuit heads reside in layers 16–30 (relative depths 0.50–0.94), wholly in the second half of the network. No head below layer 16 appears in the Llama-3.1-8B top-10. Total runtime for the 32×32 sweep (1026 patching passes plus faithfulness evaluation over 200 examples): 17,618 s (4.9 h). Data: `data/circuit-v2/patching-llama8b-v2.json`.

---

### 4.7 Cross-Model Faithfulness Evaluation

Table 7 compares circuit-level faithfulness and completeness across Llama-3.1-8B-Instruct and Pythia-1.4b, both evaluated on the v2 dataset. [[NOTE: Llama-3.2-3B was analyzed with the v1 protocol (individual patching + ablation) and was not re-run with the v2 faithfulness evaluation; direct comparison of F values to that model is not available.]]

**Table 7.** Circuit faithfulness and completeness for the top-10 activation-patching circuits in Llama-3.1-8B-Instruct and Pythia-1.4b (v2 dataset, n = 200, 15 templates). LD_circuit: logit difference when only the 10 circuit heads are active (all others ablated). LD_model∖C: logit difference when the 10 circuit heads are ablated from the full model. Data: `data/circuit-v2/`.

| Model        | Params | LD_clean | LD_circuit | LD_model∖C | F       | F_complete | Dominant head | F_leave_in |
|--------------|--------|----------|------------|------------|---------|------------|---------------|------------|
| Llama-3.1-8B | 8B     | 5.229    | 1.354      | 1.198      | **0.265**  | 0.236   | L19H23        | 0.459      |
| Pythia-1.4b  | 1.4B   | 3.888    | −0.199     | 1.239      | **−0.056** | 0.316   | L10H7         | 0.564      |

**Divergent faithfulness across models.** The qualitative result is stark: Llama-3.1-8B achieves partial positive faithfulness while Pythia's top-10 circuit has *negative* faithfulness. The Pythia circuit (LD_circuit = −0.199) does not merely fail to recover the IO preference — it reverses it, predicting the subject over the IO with confidence. The 10 heads selected by individual patching scores collectively implement the wrong computation when isolated from the rest of the model.

**Completeness is positive in both models.** Despite Pythia's negative circuit faithfulness, removing the circuit from the full model reduces performance substantially in both models ($F_	ext{complete} = 0.316$ for Pythia, $0.236$ for Llama). The selected heads are genuinely necessary to the full model's IOI computation; they are simply not sufficient — and in Pythia's case actively harmful — when run in isolation.

**Dissociation of necessity and sufficiency.** This pattern is a circuit-level analog of the head-level dissociation observed for Pythia's L1H11 (Section 4.3): a head (or set of heads) whose causal contribution to the full model is real and measurable, but whose mechanism depends on context provided by the rest of the model rather than being self-contained. For Pythia, this failure generalizes across the top-10 set: eight of the ten heads have negative minimality ΔF (removing them from the circuit improves faithfulness), meaning most selected heads interfere with the circuit's ability to perform the task in isolation. Only L10H7 (ΔF = +0.121 minimality) is a clean positive contributor. [[NOTE: add to Discussion — this is a key methodological finding; the Discussion placeholder at Section 7 should address what "circuit" identification means when the top-N heads are necessary but the top-N circuit is not sufficient.]]

**Bottleneck structure is preserved across the failure.** Even in Pythia's failing circuit, L10H7 alone achieves $F_{\{h\}} = 0.564$, well above the full circuit's F = −0.056 and above Llama-3.1-8B's full circuit F = 0.265. The single dominant head identified by patching is both correctly identified and genuinely sufficient on its own; the problem is in composing the set of 10. This suggests a practical implication: in models where faithfulness is low, the top-1 head identified by patching is more trustworthy than the top-N circuit as a unit of analysis.
...
| `data/circuit-v2/patching-llama8b-v2.json` | IOI activation patching, Llama-3.1-8B-Instruct, n=200, 15 templates |
| `data/circuit-v2/patching-pythia-v2.json` | IOI activation patching, Pythia-1.4b, n=200, 15 templates |
| `data/circuit-v2/faithfulness-llama8b-v2.json` | Circuit faithfulness + minimality, Llama-3.1-8B-Instruct, F=0.265 |
| `data/circuit-v2/faithfulness-pythia-v2.json` | Circuit faithfulness + minimality, Pythia-1.4b, F=−0.056 |
| `data/circuit-v2/joint-ablation-llama8b-v2.json` | Completeness + leave-one-in, Llama-3.1-8B-Instruct, F_complete=0.236 |
| `data/circuit-v2/joint-ablation-pythia-v2.json` | Completeness + leave-one-in, Pythia-1.4b, F_complete=0.316 |
