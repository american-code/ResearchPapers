...
Path patching is implemented within SwiftSci Interp by extending the `patch` mode to accept a target-receiver argument: when `patch_target = (l_B, h_B)` is specified, the replacement is applied only to the projection of head $A$'s clean output onto the query, key, or value input of head $B$, leaving all other paths unchanged. Results for the top-5 Llama-3.2-3B heads (10 edges, $n = 100$) are reported in Section 5.2 of this draft and in full in `submission/main.tex` (§6.2); data at `data/ioi/path-patching-llama3b-real.json`. Pythia was not path-patched. The dominant head L15H20 shows a positive direct-path score (0.165 normalized), while the L14H0 → L17H17 edge is suppressive (−0.055), consistent with a negative-name-mover interpretation.
...
### 4.6 Circuit Completeness and Minimality

We ran a joint ablation analysis to characterize the circuit as a whole: (1) ablating all ten circuit-critical heads simultaneously to measure **completeness**, and (2) ablating nine of ten heads while leaving each head active in turn to measure isolated contributions (leave-one-in). Data: `data/ioi/joint-ablation-llama3b.json`.

**Completeness.** With all ten circuit heads ablated simultaneously, the model's normalized logit difference is $F_{\text{complete}} = 0.213$ — meaning 21.3% of the clean LD range survives removing the circuit. This is slightly below the faithfulness figure of $F_{\text{faithfulness}} = 0.228$ (circuit-only direction, all non-circuit heads ablated). The close correspondence between the two directions confirms the circuit is roughly symmetric: approximately the same fraction of IOI behavior is attributable to the circuit whether approached from isolation ($F_{\text{faithfulness}}$) or complement ($F_{\text{complete}}$).

These values indicate a partial rather than complete circuit: removing the ten identified heads reduces clean IOI performance but does not collapse it, and the circuit alone does not fully account for the model's IOI capability. The remaining ~21% of clean LD in the complement likely reflects distributed contributions from heads below the top-10 threshold that accumulate but individually fall below the identification threshold.

**Leave-one-in: isolated head contributions.** When each circuit head is left active while the other nine are ablated, L17H17 produces the highest $\Delta F = +0.173$ above the fully-ablated baseline — substantially above rank-2 L26H23 at $\Delta F = 0.082$. L15H20, which is dominant under single-head ablation (ablation drop 1.578, rank 1), ranks third at $\Delta F = 0.071$. Two heads — L14H0 ($\Delta F = -0.004$) and L24H15 ($\Delta F = -0.038$) — show slightly negative values, meaning they do not individually restore performance above the fully-ablated floor when left alone.

The dissociation between L15H20's dominance under single-head ablation and L17H17's dominance in the leave-one-in is interpretively significant. L15H20's high necessity score reflects its role when the rest of the circuit is intact — removing it while all other circuit heads remain causes the largest single-head performance drop. But in isolation, L17H17 is the more capable standalone contributor. This pattern suggests L15H20's dominance is partially a function of coordinated circuit operation: it may amplify or gate contributions from other heads in a way that is not present when it operates alone.
...
**Float16 numerical stability.** A forward-pass audit over $n = 100$ IOI examples logged activation distribution quantiles (p1/p25/p50/p75/p99/max\_abs) at every layer for Llama-3.2-3B. Across all 28 layers, the audit finds zero float16 overflows and no saturation risk: max\_abs = 328 across most layers against the float16 ceiling of 65,504 (headroom of approximately 200×). The SAE-adjacent layer (layer 14) has p99 = 0.848 and max\_abs = 328, confirming activation magnitudes at the SAE extraction point are well within representable range. Audit verdict: CLEAN. Full per-layer statistics: `data/ioi/fp16_dynamic_range_audit.json`.
