...
One measurement convention is worth stating because it changed our own numbers. Every figure here is computed over the full corpus rather than read from the last training batch. The per-batch FVE logged during training is a noisy estimator that swings by several points between adjacent steps, so a final-step reading samples once from a wide distribution instead of summarising it; an earlier draft of this paper quoted those single-batch values.
