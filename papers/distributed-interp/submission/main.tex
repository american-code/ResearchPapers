...
\begin{table}[h]
\centering
\caption{SAE reconstruction quality over the full 500k-token corpus. Dead-feature counts are features that never fire anywhere in the corpus, which is a stricter and more stable criterion than the rolling 5,000-step \texttt{dead\_5k} window logged during training. Only the Mistral row is a genuine held-out measurement.}
\label{tab:quality}
\begin{tabular}{@{}llrrrrrl@{}}
\toprule
Model & Layer & Dict & Steps & MSE & FVE & Dead features & Evaluation \\
\midrule
Llama-3.2-3B          & 14 & 16,384 & 13,000$^\dagger$ & 0.00788 & 0.9865 & 3,309 (20.2\%) & in-sample \\
Mistral-7B$^\ddagger$ & 16 & 16,384 & 1,000            & 0.00415 & 0.9273 &    24 (0.1\%)  & in-sample \\
\quad \emph{(held-out)} &  &        &                  & 0.00426 & \textbf{0.9252} & 26 (0.2\%) & \textbf{held-out} \\
...
\smallskip
\noindent\footnotesize
$\dagger$~Checkpoint at step 10,000 used for all downstream analysis; training reached step 13,000 before being stopped.
$\ddagger$~4-bit quantized model weights.
\end{table}

\paragraph{Only one of these numbers is held-out, and it is the reassuring one.} The Llama and Qwen SAEs were each trained for 204 epochs over their entire 500k-token activation dump. There is no unseen data for them: every figure in their rows is in-sample, and at that level of repetition an SAE can memorise the corpus rather than learn a dictionary. We report their FVE because it is the best available characterisation, not because it establishes generalisation.

The Mistral SAE is the exception, and by accident rather than design: it was trained on only the first 50,000 tokens of its dump, leaving 450,000 unseen. Its held-out FVE is 0.9252 against an in-sample 0.9273---a gap of 0.2 percentage points. Whatever else is wrong with a 1,000-step SAE, it is not overfitting its training corpus, and the reconstruction it learned transfers. This is weak evidence that the Llama and Qwen figures are not \emph{purely} memorisation artifacts, but it is evidence from the least-trained model in the set and should not carry much weight. Collecting a held-out activation split for all three models is the single cheapest fix available to this work.

\paragraph{Dead features and dictionary collapse.} Qwen's 57.4\% dead rate is the dominant quality problem. Our training code implements no auxiliary dead-feature revival loss, so once a feature falls below the top-$k$ threshold on every batch its encoder gradient is identically zero and it can never recover. Longer training therefore monotonically worsens the dead rate: Qwen at 50,000 steps has 9,398 dead features, Llama at 13,000 steps has 3,309, and Mistral at 1,000 steps has 24. Mistral's near-zero dead rate is not a sign of health---its features simply have not had time to specialise, which shows up instead as low fingerprint support (Table~\ref{tab:inventory}). Adding the auxiliary loss of \citet{gao2024scaling} is a prerequisite for any future run.

\paragraph{Dense features dominate the sparse code.} A further pathology cuts across all three SAEs and bears directly on the safety-classifier result in \S\ref{sec:safety}. With $k = 128$ of 16,384 features active per token, a feature should fire on roughly 0.78\% of tokens if activation were spread evenly. Instead, 34 Llama features, 20 Qwen features and 34 Mistral features fire on more than half of all tokens, and the 200 most frequent features absorb 40\%, 39\% and 57\% of total activation mass respectively---1.2\% of the dictionary carrying two-fifths of the code. These are dense, low-specificity directions closer to a learned bias than to an interpretable feature. Any procedure that selects features by activation frequency will select almost exclusively from this degenerate head of the distribution.
...
The two failure modes are distinct and both diagnostic. Qwen's dictionary is half dead, the endpoint of 50,000 steps of TopK training with no auxiliary revival loss. Mistral's is barely dead but overwhelmingly low-support, the signature of an SAE stopped after 1,000 steps whose features have not yet specialised. Neither SAE is a clean instrument, and this bounds what the matching analysis can establish.
...
The null mean is the number to sit with. Two features drawn from different models with no functional relationship, matched reciprocally over a 1,000-dimensional fingerprint, correlate at 0.39--0.57 on average. The threshold of $0.8$ used in our earlier analysis is at or below that noise floor for two of the three pairs. Figure~\ref{fig:null} shows the relationship directly.
...
\midrule
\textbf{All three (universal)} & \textbf{3,753} & \textbf{0} & 0.15 & --- \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{The universality result is negative.} Under the corrected protocol, \textbf{no feature matches across all three models}. The closed-triangle count is zero, against a null expectation of 0.15 triangles per chunk-shuffled replicate (maximum observed in any null replicate: 1). The naive protocol's 3,753 ``universal'' features do not survive any one of the three corrections, let alone all three.

Pairwise, a residue survives: 10--22 matched pairs per model pair, against 5--7 expected by chance. The enrichment is real but small---roughly 2--3$\times$, or on the order of 5--15 genuine pairs out of 5,000--8,000 matchable features per model, i.e.\ 0.1--0.3\%. With counts this low the Poisson uncertainty alone spans a factor of two, and we do not think the pairwise numbers support a quantitative claim about universality rates. What they support is an upper bound: whatever shared representational vocabulary exists between these three models, this method detects it in well under 1\% of the dictionary, not the 23\% we previously reported.

\paragraph{Venn structure.} With zero universal features the three-circle Venn reduces to near-disjoint sets. Under the Pearson metric, 22 Llama features match Qwen only, 12 match Mistral only, none match both, and 16,350 of 16,384 match neither. We report this rather than a diagram because a Venn figure at these proportions would be illegible. Note that the corrected region counts partition each dictionary exactly---the naive protocol's did not, summing to 18,258 regions for a 16,384-feature Qwen dictionary, which was itself a signal that the many-to-one matching had broken the set algebra.

\paragraph{What we can and cannot conclude.} We cannot conclude that Llama-3.2-3B, Mistral-7B and Qwen2.5-3B fail to share representational structure. Three limitations block that inference, and all three point the same way. Two of the three SAEs are undertrained: the Llama SAE reached 20\% of its step target and the Mistral SAE under 2\%, on a 50k-token corpus. Half of Qwen's dictionary and two-thirds of Mistral's are unusable. And the fingerprint statistic itself---mean activation over 500-token chunks---discards within-chunk structure, so two features responding to the same rare token in different contexts are indistinguishable from two features responding to the chunk's overall topic.

What we can conclude is methodological, and it generalises past this dataset: cross-model feature matching by activation-pattern similarity requires a permutation null. Without one, the search over a large dictionary manufactures high similarities from nothing, and a threshold chosen by intuition---0.8 is the natural choice, and the one we made---can sit below the noise floor while appearing stringent. The gap between 3,753 and 0 is entirely attributable to that omission.
...
\item \textbf{Frequency bias, and the degenerate head of the distribution.} Selecting features by activation frequency selects for generalist features, not discriminative ones. This is worse than a mild bias: as \S\ref{sec:results} shows, 34 of the 200 features the labeller inspected fire on more than half of all tokens, and those 200 features carry 40\% of the entire sparse code's activation mass. The labelling pass was therefore applied almost entirely to dense, low-specificity directions---the features least likely to be about anything in particular. Selection should be by \emph{differential} activation on harmful versus safe text, and should additionally exclude features above a density ceiling.
...
\paragraph{The Llama--Mistral asymmetry disappears too.} Under the naive protocol the Llama--Mistral pair had by far the highest count (12,174; nominally 74\% coverage), and we had speculated this reflected shared architectural lineage. Under the corrected protocol Llama--Mistral yields 12 matches against Llama--Qwen's 22, reversing the ordering. The original asymmetry was an artifact of how many times individual Mistral features were double-counted, not a fact about the models.
...
\paragraph{Undertrained and collapsed dictionaries.} The Llama SAE reached 20\% of its step target and the Mistral SAE under 2\%, the latter on a 50k-token corpus. Qwen trained to completion but ended with 57.4\% of its dictionary dead, because the trainer implements no auxiliary revival loss. Only a third to a half of each dictionary is usable (Table~\ref{tab:inventory}). Any negative result about feature sharing is therefore confounded with the quality of the instruments measuring it, and we cannot separate ``these models share little'' from ``these SAEs are too degraded to detect what they share.''
