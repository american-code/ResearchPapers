...
\paragraph{How large a shared feature would we have detected? (Table~\ref{tab:mdes}).}
A null result is only as informative as the power behind it, so we measured the
procedure's sensitivity directly rather than asserting it. Taking real Mistral SAE
fingerprints as the null population, we planted $K = 50$ synthetic cross-model pairs at a
known correlation $\alpha$, ran the identical reciprocal-best-match detector, and recorded
the fraction recovered over 300 trials per point, sweeping $\alpha$ from 0.5 to 1.0 and
dictionary size from 256 to the full 8,208 matchable features.

\begin{table}[h]
\centering
\caption{Minimum detectable effect size at 80\% power. $\tau$ is the permutation-calibrated
threshold at each dictionary size; MDES is the smallest planted correlation the detector
recovers 80\% of the time. Data: \texttt{data/power-analysis/power\_table.json}, harness
\texttt{scripts/power\_analysis\_universality.py}.}
\label{tab:mdes}
\begin{tabular}{@{}rrrr@{}}
\toprule
...
\textbf{MDES tracks $\tau$ almost exactly, and this is the central limitation of the
method.} Because the threshold must be set above a null whose mean rises with dictionary
size, the detector only recovers planted pairs whose correlation is at or above that
threshold---$\alpha \geq 0.95$ at small dictionaries, and effectively $\alpha = 1.0$ at the
full 8,208. A genuinely shared feature realised at correlation 0.8, which most
practitioners would regard as a strong correspondence, is invisible.

This reframes the negative result precisely. ``Zero universal features'' does not mean
these three models share no representational structure. It means \emph{no feature triple
is shared at correlation $\geq 0.95$}, which is a far weaker statement and the only one
the data supports. Any shared vocabulary below that correlation would have gone
undetected, and we cannot bound how much of it there is.

The estimate is conservative in one direction: the simulation ran at 100 chunks against
the 1,000 used in the main analysis, so its fingerprints are noisier, $\tau$ is higher, and
the true MDES at full resolution may be somewhat lower. It is not conservative enough to
change the conclusion---detection remains confined to near-perfect correspondence.
...
PKU-SafeRLHF~\citep{ji2023beavertails}, 600 examples at 44.8\% prevalence, and added a
baseline the earlier evaluation lacked: an LLM judge (Llama-3.2-3B-Instruct, 4-bit) scored
by the Yes-minus-No logit difference on ``Is this AI response harmful?''
...
directions does not beat simply asking an instruction-tuned model. That is the honest
comparison, and the earlier draft had no baseline at all against which to make it.
...
\paragraph{Detection floor on universality.} The matching procedure's minimum detectable effect size is a correlation of 0.95--1.00 (\S\ref{sec:matching}). Our null result therefore bounds shared structure only above that correlation; a shared vocabulary realised at, say, 0.8 would be invisible to this method and we cannot say how large it is. Lowering the floor requires a similarity statistic whose null does not rise with dictionary size---higher-resolution fingerprints help, but the extreme-value problem is structural, not merely a matter of more tokens.
