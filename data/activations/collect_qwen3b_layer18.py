"""
Collect residual stream activations from Qwen2.5-3B at 50% layer depth.

Qwen2.5-3B has 36 layers (0-indexed 0-35). 50% depth = layer 18.
Output directory: data/activations/qwen-3b-layer18/

Output files:
  activations.npy   -- float16 memmap, shape [n_tokens, 2048]
...
TARGET_LAYER  = 18          # layer 18 of 36 = 50% depth
TARGET_TOKENS = 500_000
CHUNK_TOKENS  = 512
HIDDEN_SIZE   = 2048
OUT_DIR       = Path(__file__).parent / "qwen-3b-layer18"
...
assert num_layers == 36, f"Expected 36 layers for Qwen2.5-3B, got {num_layers}"
assert TARGET_LAYER < num_layers

# ── Verify hidden size matches ────────────────────────────────────────────────
actual_hidden = qwen_model.args.hidden_size
assert actual_hidden == HIDDEN_SIZE, \
    f"Expected hidden_size={HIDDEN_SIZE}, got {actual_hidden}"

# ── Corpus: same as Llama run — wikitext-103-raw-v1 train ────────────────────
