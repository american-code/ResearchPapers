...
OUT_DIR   = WORKSPACE / "data/seed-stability-newcorpus"
...
ARMS = {
    "seed42":  {"ckpt": SAE_DIR / "llama3b-l14-bf16-sae/checkpoint_final.npz",
                "acts": ACTS_DIR / "llama3b-l14-bf16"},
    "seed123": {"ckpt": SAE_DIR / "llama3b-l14-bf16-seed123-sae/checkpoint_final.npz",
                "acts": ACTS_DIR / "llama3b-l14-bf16"},
}
PAIRS = (("seed42", "seed123"),)
...
def main() -> None:
    ...
    print("Matched seed control, same corpus -- seed 42 vs seed 123")
    ...
    (OUT_DIR / "seed-newcorpus-report.json").write_text(json.dumps(out, indent=1))
    print(f"\nwrote {OUT_DIR/'quant-interp-report.json'}  ({out['elapsed_s']}s)")

    r = out["metrics"]["pearson"]["bf16_vs_q4"]
    print("\n\u2500 headline \u2500")
    print(f"  bf16 vs 4-bit: {r['matches_calibrated']} matched, "
          f"{r['pct_of_matchable']}% of matchable, {r['enrichment_over_null']}x over null")
    print( "  compare: bf16-vs-4bit 7.7% (same corpus); old-corpus seed ceiling 7.9-8.3%")
