#!/bin/bash
# Waits for both seed-stability SAEs to finish, then runs the matching analysis.
...
A=data/sae-runs/llama-3b-layer14-seed123/checkpoint_final.npz
B=data/sae-runs/llama-3b-layer14-seed456/checkpoint_final.npz
...
while true; do
  ...
  if ! pgrep -f "[t]rain_sae.py" > /dev/null && ! pgrep -f "[r]un_seed_stability.sh" > /dev/null; then
    ...
    echo "  last runner output:"; tail -5 data/sae-runs/seed-stability.log
    exit 1
  fi
...
python3 data/sae-analysis/seed_stability_match.py
rc=$?
